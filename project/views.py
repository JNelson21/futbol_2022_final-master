...
def contextoListaJugadoresPorGoles(request):
    # competencia_seleccionada = competicion.objects.get(nombre=nombre_competicion.upper()) 
    #filtro_encuentros_competencia = encuentro.objects.filter(competicion_id=competencia_seleccionada.competicion_id)
    #persona_jugador = persona.objects.filter(tipo_persona_id=1)
    #lista_goles_persona = []
    ...
    #result = evento_persona.objects.all().filter(evento_id=9).values('persona_id').annotate(count=Count('persona_id')).order_by()
    resulta = evento_persona.objects.filter(evento_id=9).values('persona_id').annotate(count=Count('persona_id')).order_by()
    # evento_persona  = evento_persona.objects.raw('SELECT persona.nombre, count(event.) FROM evento_persona inner join persona on evento_persona.persona_id = persona group by evento_persona_id')
    ...
    data={
        'result': resulta,
        'jugador_goles': lista2
    }
    return render(request, 'lista_jugadores_goles.html', data)

def contextoListaJugadoresPorAmarillas(request):
    resulta = evento_persona.objects.filter(evento_id=1).values('persona_id').annotate(count=Count('persona_id')).order_by()   
    lista2= []
    for r in resulta :
        li = persona.objects.all().filter(persona_id = r.get('persona_id'))
        for v in li :
    ...
    data={
        'result': resulta,
        'jugadores_amarillas': lista2
    }
    return render(request, 'lista_jugadores_amarillas.html', data)
def contextoListaJugadoresPorRojas(request):
    resulta = evento_persona.objects.filter(evento_id=2).values('persona_id').annotate(count=Count('persona_id')).order_by()   
    lista2= []
    for r in resulta :
        li = persona.objects.all().filter(persona_id = r.get('persona_id'))
        for v in li :
